...
export async function createMergeRequest(options: {
  ...
  const { config, projectId, sourceBranch, targetBranch, title, description, labels, reviewerIds } = options;
  ...
  const response = await fetch(`${config.baseUrl.replace(/\/$/, '')}/api/v4/projects/${projectId}/merge_requests`, {
    method: 'POST',
    headers: {
