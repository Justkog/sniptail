...
export async function runCommand(cmd: string, args: string[], opts: RunOptions = {}): Promise<RunResult> {
  const start = Date.now();
  const { cwd, env, echo = false, logFilePath, timeoutMs = 0, signal, redact = [], allowFailure = false } =
    opts;
