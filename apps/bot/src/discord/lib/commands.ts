...
export function buildCommandNames(prefix: string) {
  return {
    ...
    run: `${prefix}-run`,
    agent: `${prefix}-agent`,
    agentSessions: `${prefix}-agent-sessions`,
...
export function buildDiscordCommandDefinitions(botName: string) {
  ...
  const commands = [
    ...
    {
      name: names.agent,
      description: 'Start a freeform coding-agent session in a Discord thread',
      type: 1,
      options: [
        {
          name: 'prompt',
          description: 'Initial prompt for the coding agent',
          type: 3,
          required: true,
        },
        ...
        {
          name: 'agent_profile',
          description: 'Coding-agent profile key',
          type: 3,
          required: false,
          autocomplete: true,
        },
    ...
    {
      name: names.agentSessions,
      description: 'Browse and attach previous coding-agent sessions',
      type: 1,
      options: [
        {
          name: 'worker',
          description: 'Worker id that owns the provider sessions',
          type: 3,
          required: true,
          autocomplete: true,
        },
        {
          name: 'agent_profile',
          description: 'Optional coding-agent profile key',
          type: 3,
          required: false,
          autocomplete: true,
        },
        {
          name: 'workspace',
          description: 'Optional workspace key filter',
          type: 3,
          required: false,
          autocomplete: true,
        },
