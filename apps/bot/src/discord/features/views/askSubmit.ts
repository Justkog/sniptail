...
export async function handleAskModalSubmit(
  ...
  try {
    ...
    logger.warn({ err }, 'Failed to load Discord command context files for ask job');
  ...
  if (acceptance.acceptancePosted) {
    try {
      ...
      logger.warn({ err, jobId: job.jobId }, 'Failed to delete ephemeral reply after job acceptance');
    }
    return;
  }
