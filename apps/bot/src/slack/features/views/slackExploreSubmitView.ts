...
export function registerSlackExploreSubmitView({
  app,
  slackIds,
  config,
  queue,
  ...
  app.view(slackIds.actions.exploreSubmit, async ({ ack, body, view, client }) => {
    ...
    try {
      ...
      logger.warn({ err }, 'Failed to load Slack modal context files for explore job');
    ...
    if (!repoKeys.length) {
      await postMessage(app, {
        channel: metadata?.channelId ?? body.user.id,
        text: `Please select at least one repo for ${slackIds.commands.explore}.`,
      });
      return;
    }
