...
- `/sniptail-agent`: Starts a freeform coding-agent session in a Discord thread with follow-ups, stop/steer controls, and interactive permission or question prompts, including ACP-backed profiles when configured. In multi-worker deployments, live controls are routed back to the worker that owns the session.
- `/sniptail-agent-sessions`: Browses previous coding-agent sessions on a selected worker and attaches one to a new Slack or Discord thread without immediately sending a prompt.
